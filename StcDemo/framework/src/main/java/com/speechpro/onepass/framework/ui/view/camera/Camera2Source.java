...
import com.speechpro.onepass.framework.ui.view.camera.callbacks.AutoFocusCallback;
...
public class Camera2Source {
     ...
     *
     * @param quality       low or medium
     * @param videoFile     video path
     * @param videoCallback the callback for video recording start, stop or error
     */
    public void recordVideo(@NonNull CameraQuality quality, @NonNull String videoFile, @NonNull VideoCallback videoCallback) {

        int width = 640;
        int height = 480;

        if (quality == CameraQuality.LOW) {
            width = 320;
            height = 240;
        }
        try {
            ...
            if (swappedDimensions) {
                mMediaRecorder.setOrientationHint((INVERSE_ORIENTATIONS.get(mDisplayOrientation) - adjustment) % 360);
            } else {
                mMediaRecorder.setOrientationHint((ORIENTATIONS.get(mDisplayOrientation) - adjustment) % 360);
            }
    ...
    public void stopVideo() {
        ...
        videoCallback.onVideoStop(videoFile);
        closePreviewSession();
    }
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback = new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session, @NonNull CaptureRequest request, @NonNull TotalCaptureResult result) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        if (mCaptureSession != null && mCaptureSession.isReprocessable()) {
                            unlockFocus();
                        }
                    } else {
                        unlockFocus();
