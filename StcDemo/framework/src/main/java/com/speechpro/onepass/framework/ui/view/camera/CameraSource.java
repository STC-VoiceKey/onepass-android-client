...
import com.speechpro.onepass.framework.ui.view.camera.callbacks.AutoFocusMoveCallback;
...
public class CameraSource {
    ...
    */
    public boolean canRecordVideo(@VideoMode int videoMode) {
        try {
            get(getIdForRequestedCamera(mFacing), videoMode);
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    */
    ...
    public void recordVideo(@NonNull CameraQuality quality, @NonNull String videoFile, @NonNull VideoCallback videoCallback) {
        ...
        if (!checkCamera()) {
            this.videoCallback.onVideoError("Camera Error");
            return;
        }
        ...
        CamcorderProfile profile = getNecessaryProfile(cameraId, quality);
        ...
        try {
            ...
            this.videoCallback.onVideoError(e.getMessage());
            return;
        } catch (IOException e) {
            releaseMediaRecorder();
            this.videoCallback.onVideoError(e.getMessage());
    ...
    public void stopVideo() {
        releaseMediaRecorder();
        this.videoCallback.onVideoStop(videoFile);
    }
    ...
    private CamcorderProfile getNecessaryProfile(int cameraId, CameraQuality quality) {
        return get(cameraId, chooseOptimalProfile(cameraId, quality));
    }

    private int chooseOptimalProfile(int cameraId, CameraQuality cameraQuality) {
        List<Integer> qualityLevels = new ArrayList<>();
        if (cameraQuality == CameraQuality.MEDIUM) {
            qualityLevels.add(QUALITY_720P);
        }
        qualityLevels.add(QUALITY_480P);
        qualityLevels.add(QUALITY_CIF);
        qualityLevels.add(QUALITY_QCIF);
        qualityLevels.add(QUALITY_HIGH);
        qualityLevels.add(QUALITY_LOW);

        for (int quality : qualityLevels) {
            if (hasProfile(cameraId, quality)) {
                Log.d(TAG, "chooseOptimalProfile: " + quality);
                return quality;
            }
        }
        return 0;
